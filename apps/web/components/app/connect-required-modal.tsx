...
> = {
  gmail: {
    ...
    body: "This email stays in your Queue until you approve it. Connect Gmail in Settings to deliver messages from MailOS.",
...
> = {
  gmail: {
    ...
    primaryLabel: "Sign in to use MailOS",
    icon: Mail,
  },
  calendar: {
...
export function useQueueIntegrationGate(email: string | null | undefined) {
  ...
  const modal = requirement ? (
    <ConnectRequiredModal
      ...
      isDemoUser={demo.isDemo}
      onCloseAction={() => setRequirement(null)}
    />
  ) : null;
