...
export function ThreadAppShell({ children }: { children: ReactNode }) {
  ...
  if (isLoading || isError || !user) {
    return (
      <div className="thread-page thread-app-splash" suppressHydrationWarning>
        <div className="thread-app-spinner" aria-label="Loading" suppressHydrationWarning />
      </div>
    );
  }

  const meta = PAGE_META[pathname ?? "/inbox"] ?? { title: "MailOS", sub: "" };

  return (
    <div className="thread-page thread-app" suppressHydrationWarning>
      {mobileNavOpen ? (
        <button
          type="button"
      ...
      <aside className="thread-app-side" data-open={mobileNavOpen ? "true" : undefined}>
        <div className="thread-app-side-head">
          <Link href="/inbox" className="thread-app-side-brand" onClick={() => setMobileNavOpen(false)}>
            <Image src="/thread-logo.svg" alt="MailOS" width={24} height={24} priority />
        ...
        <div className="thread-app-user" ref={menuRef}>
          ...
          <button type="button" className="thread-app-user-btn" onClick={() => setMenuOpen((v) => !v)}>
            ...
            <span className="thread-app-user-meta">
              <span className="thread-app-user-name">{user.displayName || user.fullName || "MailOS user"}</span>
            </span>
