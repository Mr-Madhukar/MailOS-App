...
const COMMANDS = [
  { icon: <Calendar size={15} />, label: "Go to Calendar", keys: ["G", "C"], key: "cal" },
  { icon: <PenLine size={15} />, label: "Compose new message", keys: ["C"], key: "compose" },
  { icon: <Sparkles size={15} />, label: "Ask the agent", keys: ["G", "A"], key: "agent" },
    ...
    <div className="lp-row" data-unread={m.unread} data-active={active}>
      ...
      <span className="lp-row-main">
        ...
        <span className="lp-row-sub">
          {m.star && <Star size={12} fill="currentColor" />}
          {m.subject}
        </span>
      </span>
    </div>
...
/** The hero centrepiece: an alive MailOS window — the inbox with a moving J/K
...
function HeroWindow() {
  ...
  useEffect(() => {
    const selId = setInterval(() => setSel((s) => (s + 1) % INBOX_ITEMS.length), 1500);
    const word = "cal";
    let i = 0;
    let dir = 1;
    ...
    const tick = () => {
      ...
      if (dir > 0) {
        if (i < word.length) { i += 1; t = setTimeout(tick, 220); }
        else { dir = -1; t = setTimeout(tick, 2200); }
      } else if (i > 0) { i -= 1; t = setTimeout(tick, 110); }
      else { dir = 1; t = setTimeout(tick, 900); }
    };
    t = setTimeout(tick, 1100);
    return () => { clearInterval(selId); clearTimeout(t); };
  }, []);
...
const CAL = [
  ...
  { d: "Wed", e: [] as { t: number; h: number; l: string }[] },
  ...
  return (
    <div className="lp-mini lp-cal">
      <div className="lp-cal-head">
        {CAL.map((c) => (
          <span key={c.d} className="lp-cal-dayname">{c.d}</span>
        ))}
      </div>
...
function Spotlight() {
  return (
    <svg className="lp-spotlight" viewBox="0 0 3787 2842" fill="none" aria-hidden="true">
      ...
      <defs>
        <filter
          id="lp-spot-blur" x="0.860352" y="0.838989"
...
export function MailosLanding() {
  ...
  return (
    <div className="lp thread-page" suppressHydrationWarning>

      <nav className="lp-nav" ref={navRef} data-scrolled="false">
        <span className="lp-brand">
          <ThreadLogoMark size={18} />
        ...
        {user ? (
          <Link href="/inbox" className="lp-btn lp-btn-primary lp-nav-cta">
            Open MailOS <ArrowRight size={14} />
          </Link>
        ) : (
          <button
            type="button"
      ...
      <main>
        ...
        <section className="lp-hero" id="start" ref={heroRef}>
          <Spotlight />
          <Motes />
          <span className="lp-rise lp-glyph" style={{ animationDelay: "0s" }}>
            <video
              src="/mascot.webm"
              autoPlay
              loop
          ...
          <div className="lp-cta-row lp-rise" style={{ animationDelay: "0.24s" }}>
            <button
              type="button"
              className="lp-btn lp-btn-primary lp-cta"
              onClick={handleConnect}
            >
              {user ? "Open MailOS" : "Connect Gmail"} <ArrowRight size={16} />
            </button>
            <a href="#features" className="lp-btn lp-cta lp-cta-ghost">
              See it fly
            </a>
          </div>
        ...
        <section className="lp-features" id="features">
          ...
          <div className="lp-bento">
            ...
            <BentoCard
              ...
              body="Start typing and MailOS surfaces the command — compose, search, schedule, run the agent."
              keys={["⌘", "K"]}
            >
              <MiniPalette />
            </BentoCard>
        ...
        <section className="lp-section" id="trust">
          ...
          <div className="lp-trust">
            <div className="lp-trust-row lp-rise">
              ...
              <p>Connects through Google OAuth. MailOS never sees your password, and you can revoke access from your Google account at any time.</p>
            </div>
            <div className="lp-trust-row lp-rise">
        ...
        <section className="lp-section" id="faq">
          ...
          <div className="lp-faq lp-rise">
            ...
            <details>
              <summary>What does MailOS cost?</summary>
            ...
            <details>
              ...
              <p>MailOS is in beta and Google&apos;s verification is in progress. Choose Advanced, then Continue — the permissions requested are only Gmail and Calendar.</p>
            </details>
          </div>
        </section>
        ...
        <section className="lp-final">
          <div className="lp-final-inner lp-rise">
            <span className="lp-glyph"><ThreadLogoMark size={56} /></span>
            <h2>Take control of your inbox.</h2>
